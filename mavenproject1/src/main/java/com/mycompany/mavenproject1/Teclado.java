...
public class Teclado {
    ...
    public static LocalTime comphora(String cadena){
        LocalTime DT=null;
        boolean comp=false;
        do{
             try {
            ...
            DT = LocalTime.parse(new Scanner(System.in).nextLine(), f);
            comp = true;
        } catch (java.time.format.DateTimeParseException e) {
            System.out.println("Introduce una hora válida con el formato HH:mm (por ejemplo: 14:30)");
        }
        }while(!comp);
        return DT;
    }
